                        ...
                        productData.length && productData.map((item) => (
                            <div key={item.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-4">
                                ...
                                <div className="flex items-center justify-between gap-3">
                                    <button
                                        onClick={() => {
                                            dispatch(removeItem(item)); toast.error("Removed from cart");
                                        }}
                                        className="px-3 bg-gray-300  hover:bg-gray-600 dark:bg-gray-700 rounded">-</button>
                                    <span className="text-gray-800 dark:text-white ">{getQty(item.id)}</span>
                                    <button onClick={() => { dispatch(addItem(item)); toast.success("Added to cart"); }} className="px-3 bg-gray-300 hover:bg-gray-600 dark:bg-gray-700 rounded ">+</button>
                                </div>
                            </div>
                        ))
                    )}
